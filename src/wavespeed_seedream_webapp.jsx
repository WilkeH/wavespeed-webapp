...
const extractImageUrlsFromResponse = (response: any): string[] => {
  if (!response) return [];

  const data = response.data ?? response;
  const outs = (data as any)?.outputs || (data as any)?.data?.outputs;
...
const runExtractImageUrlsFromResponseTests = () => {
  try {
    ...
    // zusätzlicher Test: völlig anderes Shape, keine outputs -> leeres Array
...
type GalleryItem = {
  url: string;
  version: "v4" | "v4.5" | "both";
};

// Lupe-Status: Position relativ zum Container (für die Anzeige)
// und relativ zum Bild (für das korrekte Zoom-Mapping)
type LensState = {
  containerX: number;
  containerY: number;
  imageX: number;
...
export default function WavespeedSeedreamEditApp() {
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [apiKey, setApiKey] = useState<string>("");
  const [keyFlash, setKeyFlash] = useState<"wilke" | "kim" | null>(null);
  const [prompt, setPrompt] = useState<string>("");
  const [images, setImages] = useState<string[]>([]);
  const [size, setSize] = useState<string>("3072*4096");
  const [selectedRatio, setSelectedRatio] = useState<string | null>(null);
  const [orientation, setOrientation] = useState<Orientation>("portrait");
  const [longSide, setLongSide] = useState<string>("4096");
  const [isLoading, setIsLoading] = useState(false);
  const [activeButton, setActiveButton] =
    useState<"v4" | "v4.5" | "both" | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [rawResponse, setRawResponse] = useState<any>(null);
  const [galleryOutputs, setGalleryOutputs] = useState<GalleryItem[]>([]);
  const [lensActive, setLensActive] = useState(false);
  const [lensPos, setLensPos] = useState<LensState | null>(null);
  ...
  const computeSizeFromRatio = (ratioW: number, ratioH: number) => {
  ...
  const handleSubmitEndpoint = async (version: "v4" | "v4.5" | "both") => {
    setError(null);
    setRawResponse(null);
    ...
    const endpoints: Record<"v4" | "v4.5", string> = {
    ...
    const runRequest = async (url: string, tagVersion: "v4" | "v4.5") => {
      const body = {
        prompt,
  ...
  const handleLensMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!lensActive || !imageContainerRef.current || !imageRef.current || !selectedImage) {
      return;
    }
  ...
  // Lens-Style vorbereiten (damit kein style={() => ...} im JSX steht)
      ...
      {keyFlash && (
        ...
        >
          <div className="text-3xl font-bold tracking-wide drop-shadow-xl">
            {keyFlash === "wilke" ? "Wilke API aktiviert" : "Kim API aktiviert"}
          </div>
        </div>
      )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start mt-1">
                <div className="flex flex-col gap-2">
                  <span className="text-[11px] text-slate-400">Längste Seite</span>
                  <input
                    type="number"
                    min={256}
                    max={8192}
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-[11px] text-slate-400">Ausrichtung</span>
                  <button
                    type="button"
                    onClick={() =>
                      setOrientation((o) =>
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-[11px] text-slate-400">Seitenverhältnis</span>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Aktuelle Größe: <span className="text-indigo-400">{size}</span>
              </p>
            </div>
        ...
        <div className="flex flex-col gap-4">
                ...
                {galleryOutputs.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[11px] text-slate-400">{labelText}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-xl max-h-72 overflow-auto">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] text-slate-500">Debug / Fallback</span>
            </div>
            {rawResponse ? (
              ...
              <p className="text-xs text-slate-500">
                Noch kein Ergebnis. Sende einen Request, um die JSON-Antwort hier zu sehen.
              </p>
            )}
          </div>
        </div>
